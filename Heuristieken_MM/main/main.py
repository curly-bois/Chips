...
while counter < 1:
    ...
    new_unconnected_sets = unconnected_sets
    new_connected_sets = connected_sets

    while len(new_unconnected_sets) > 0 and hilltries < 10:
        np.random.shuffle(connected_sets)
        hilltries += 1
        new_connections = []
        broken_sets = []

        for set in new_unconnected_sets:
            new_connections.append(set)

        # sets_to_be_broken = int(len(connected_sets) * 0.2)
        sets_to_be_broken = int(len(new_connected_sets) * 0.4)
        if len(new_connected_sets) < 3:
            sets_to_be_broken = 1
        for i in range(sets_to_be_broken):
            new_connected_sets[i].disconnect()
            new_connections.append(new_connected_sets[i])
            broken_sets.append(new_connected_sets[i])
